...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  ALTER TABLE "clients" ADD CONSTRAINT "clients_industry_id_categories_id_fk" FOREIGN KEY ("industry_id") REFERENCES "public"."categories"("id") ON DELETE set null ON UPDATE no action;
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.execute(sql`
  ALTER TABLE "ads" DROP CONSTRAINT "ads_client_id_clients_id_fk";
  
  ALTER TABLE "_ads_v" DROP CONSTRAINT "_ads_v_version_client_id_clients_id_fk";
